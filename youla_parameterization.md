...
- $Q(s)$ receives $r - (y - \hat{y})$ and produces the control signal $u$
- $u$ splits: one copy drives the real plant $G(s)$, the other drives an internal model $\tilde{G}(s)$
- $\tilde{G}(s)$ predicts what the plant *should* produce: $\hat{y} = \tilde{G}(s) \cdot u$
- The feedback signal is $y - \hat{y}$ — the difference between actual and predicted output
- That difference is subtracted from the reference $r$

If the model is perfect ($\tilde{G} = G$), then $y - \hat{y} = d$, the disturbance at the plant output. The feedback path carries **only the disturbance**. The reference $r$ passes through $Q$ and $G$ in a pure feedforward path — no feedback loop at all!
