...
global.numeric = {
  ...
  inv: M => { const n=M.length; const aug=M.map((row,i)=>[...row,...Array.from({length:n},(_,j)=>i===j?1:0)]); for(let c=0;c<n;c++){let p=c;for(let r=c+1;r<n;r++)if(Math.abs(aug[r][c])>Math.abs(aug[p][c]))p=r;[aug[c],aug[p]]=[aug[p],aug[c]];if(Math.abs(aug[c][c])<1e-12)return null;const d=aug[c][c];for(let j=c;j<2*n;j++)aug[c][j]/=d;for(let r=0;r<n;r++){if(r===c)continue;const f=aug[r][c];for(let j=c;j<2*n;j++)aug[r][j]-=f*aug[c][j];}}return aug.map(row=>row.slice(n)); },
