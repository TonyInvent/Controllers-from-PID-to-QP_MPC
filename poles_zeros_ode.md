...
When you drag the $K_p$ slider and watch the closed-loop poles march vertically (reducing $\zeta_{\text{eff}}$), you are NOT changing the decay rate $\sigma$. The real part $-\zeta\omega_n$ is fixed — $K_p$ only enters the discriminant under the square root (see Section 5). What changes is the imaginary part $\omega_d$, which grows with $K_p$. This makes $\zeta_{\text{eff}} = \sigma / \sqrt{\sigma^2 + \omega_d^2}$ drop and oscillation more visible, but the exponential decay envelope $e^{-\sigma t}$ stays exactly the same.
...
The right-hand side now involves the **derivative** of the input. For a step input ($u(t)=1$, $\dot{u}(t)=0$ for $t > 0$), the forcing term simplifies to $z \cdot 1 = z$. The ODE becomes:

$$\ddot{y} + 2\zeta\omega_n\dot{y} + \omega_n^2 y = z$$

To find the **particular solution** (the forced steady-state response), assume a constant $y_p$: then $\dot{y}_p = 0$ and $\ddot{y}_p = 0$. Substitute:

$$0 + 0 + \omega_n^2 y_p = z \quad\Longrightarrow\quad y_p = \frac{z}{\omega_n^2}$$

Equivalently, from the transfer function: $G(0) = \frac{0 + z}{0^2 + 2\zeta\omega_n\cdot 0 + \omega_n^2} = \frac{z}{\omega_n^2}$, which is the **DC gain** from $u$ to $y$. The particular solution is the DC gain times the step amplitude.
