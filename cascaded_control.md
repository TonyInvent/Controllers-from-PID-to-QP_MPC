...
This is not an empirical guideline. It falls out of a single requirement: **the effective damping ratio of the outer loop must remain above 0.7 when the inner loop's closed-loop dynamics are included in the outer loop's plant.**

### 6.1 Set up the problem

Consider two loops in cascade. The inner loop has been designed as a well-damped second-order system — PI controller on a first-order plant produces exactly this:

$$T_{\text{inner}}(s) = \frac{\omega_{n,\text{in}}^2}{s^2 + 2\zeta_{\text{in}} \omega_{n,\text{in}} s + \omega_{n,\text{in}}^2}$$

The inner loop is tuned for $\zeta_{\text{in}} \approx 0.7$–$1.0$. Its bandwidth (crossover frequency) is approximately $\omega_{c,\text{in}} \approx \omega_{n,\text{in}}$ for this damping range. Its time constant — the inverse of the magnitude of its slower pole — is approximately $\tau_{\text{in}} = 1 / \omega_{c,\text{in}}$.

The outer loop's plant includes this inner closed-loop transfer function. For the position-over-velocity cascade, the outer plant is an integrator preceded by the velocity loop:

$$G_{\text{outer}}(s) = T_{\text{inner}}(s) \cdot \frac{1}{s}$$

A proportional controller $C_{\text{outer}}(s) = K_p$ is applied. (This is the standard position-loop design — see §5.2.)
...
A well-damped inner loop behaves approximately as a first-order lag at frequencies well below its bandwidth:

$$T_{\text{inner}}(j\omega) \approx \frac{1}{1 + j\omega / \omega_{c,\text{in}}} \quad \text{for } \omega \ll \omega_{c,\text{in}}$$

At the outer loop's crossover $\omega_{c,\text{out}} = \omega_{c,\text{in}} / N$, the inner loop contributes phase lag:

$$\Delta\phi = -\arctan\!\left(\frac{\omega_{c,\text{out}}}{\omega_{c,\text{in}}}\right) = -\arctan(1/N)$$

| $N$ | Phase loss at outer crossover | Impact |
|-----|------------------------------|--------|
| 2 | $-26.6^\circ$ | Eats heavily into phase margin — outer loop sluggish or unstable |
| 3 | $-18.4^\circ$ | Significant — need extra PM budget |
| 5 | **$-11.3^\circ$** | **Acceptable — easily absorbed in a standard 45°–60° PM design** |
| 10 | $-5.7^\circ$ | Negligible |

At $N = 5$, the inner loop costs only $11^\circ$ of phase at the outer crossover — a loss you can absorb without redesign. At $N = 3$, you lose $18^\circ$ — you'd need to deliberately over-design the outer loop's phase margin to compensate. At $N = 2$, the loss is $27^\circ$, and independent design breaks down entirely.

### 6.6 The practical numbers

Both the damping-ratio derivation and the phase-loss check converge on $N \geq 5$. In practice:
...
At $N=5$, each inner loop is fast enough to be a clean first-order approximation for the next outer loop. At $N=10$, you have headroom for robustness against parameter drift. Below $N=3$, the approximations break, the simple tuning procedure stops working, and you must design the loops jointly — which is exactly what LQR does with its full state-feedback gain matrix.
