...
## 6. The bandwidth separation rule
...
In practice, 5× is the minimum. 8–10× is common in well-tuned drives. Why this number?

Consider a position loop at 20 Hz bandwidth (typical for a CNC axis). The velocity loop must be at least 100 Hz, and the current loop at least 500 Hz. With 5× separation:

| Loop | Bandwidth | Time constant |
|------|-----------|---------------|
| Current | 500 Hz | 0.32 ms |
| Velocity | 100 Hz | 1.6 ms |
| Position | 20 Hz | 8.0 ms |

At the 10× end:

| Loop | Bandwidth | Time constant |
|------|-----------|---------------|
| Current | 2000 Hz | 0.08 ms |
| Velocity | 200 Hz | 0.8 ms |
| Position | 20 Hz | 8.0 ms |

The separation ensures that when the outer loop sends a command, the inner loop settles so quickly that the outer loop sees its closed-loop response as a nearly-ideal low-order system. The 5× rule comes from Bode's loop-shaping principle: a lag at one-fifth the crossover frequency introduces negligible phase loss (< 11°) at the crossover. The inner loop's dynamics are "transparent" to the outer loop's design.

What happens if the separation is too small? The inner loop's phase lag starts to eat into the outer loop's phase margin. A 10°–15° loss might be tolerable. A 30° loss makes the outer loop sluggish or unstable. The cascade degrades gracefully as the separation shrinks, but the simple "design each loop independently" procedure stops working.
