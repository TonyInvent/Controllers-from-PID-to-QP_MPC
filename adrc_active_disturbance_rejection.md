...
**Where $l_1 = 2\omega_o, l_2 = \omega_o^2$ comes from.** Start with the plant model used inside the ESO and the observer equations side by side:
...
The ESO, tuned to $\omega_o = 20$ rad/s, now tracks $f$ in real time. It continuously outputs $\hat{f}$, an estimate of the total disturbance. **But having $\hat{f}$ is only half the story.** The other half is using it. How? By cancelling it at the input.

Start from the ADRC plant equation $\dot{y} = f + b_0 u$. ADRC introduces a **virtual control** $u_0$ — the control you *want* the plant to feel after the disturbance is cancelled. The desired nominal dynamics are a pure integrator:
...
### 6.5 Robotics: useful, but not where the current fashion points

Robotics sounds like the perfect home for ADRC. A robot joint is full of things that engineers hate modeling exactly: gear friction, cable compliance, temperature-dependent motor constants, unmodeled link coupling, payload changes, and contact impulses from the floor or the world. The usual rigid-body equation says:

$$M(q)\ddot{q} + C(q,\dot{q})\dot{q} + g(q) + \tau_f + \tau_{\text{contact}} = \tau$$

If you are controlling one joint, almost everything on the left-hand side is "someone else's problem." The coupling torque from the other joints, the gravity term, the unknown payload, the floor impact — all of it can be lumped into $f$. This is exactly the ADRC move: estimate the joint's total disturbance with an ESO, cancel it, and leave a simple double integrator for the outer position loop. This is why ADRC papers in robotics are not hard to find: robot manipulators, mobile robots, aerial manipulators, quadrotors, and flexible-joint systems all benefit from a controller that does not ask for a perfect dynamics model.

So why don't modern legged and humanoid robots simply run ADRC at every joint?

Because the dominant learned-locomotion stack has already made a different bargain. In public sim-to-real systems such as `legged_gym` and Humanoid-Gym, the learned policy usually runs at a lower rate than the motor servo and outputs an action that is interpreted as a joint target or target offset. A high-rate low-level controller then converts that target into torque:

$$\tau = K_p(q_{\text{des}} - q) - K_d\dot{q}$$

sometimes with velocity targets, torque actions, or residual/feedforward terms depending on the implementation. The important point is not the exact formula. The important point is the interface: the learning system is trained together with a simple, predictable impedance layer. The PD loop is not merely an old-fashioned placeholder. It is part of the plant that the policy learns to use.

That changes the role of ADRC. If an ESO is inserted underneath the policy, it does not just "improve the joint controller." It changes the closed-loop actuator seen by the policy. The policy was trained against a particular impedance, saturation behavior, delay, and contact response. ADRC may reject disturbances better in the classical servo sense, but it may also remove exactly the compliant error dynamics that the learned gait uses for balance. In walking, a foot strike is not merely a disturbance to erase; it is also information and energy exchange with the environment.

There are three practical frictions.

**1. The policy already learned through the low-level controller.** In position-action RL, the neural network does not output a desired motor voltage or current. It outputs something like "make this joint a little more flexed," and the PD layer turns that into physical impedance. Domain randomization then teaches the policy to survive mass variation, friction variation, latency, pushes, and terrain. ADRC would estimate some of those variations as $f$ and cancel them, but the policy may already have learned a behavior that assumes the uncancelled residual dynamics.

**2. Contact is not a smooth disturbance.** ADRC works best when the total disturbance is observable and changes within the observer's bandwidth. Robot contact often violates that assumption. Foot impacts, stick-slip, backlash, torque limits, and ground transitions can be discontinuous or constraint-dominated. Raising $\omega_o$ helps tracking, but it also amplifies encoder noise and collision transients. In a saturated actuator, the cancellation term cannot actually be applied, and the ESO must be carefully fed the saturated torque to avoid observer windup.

**3. Statefulness is expensive at the policy boundary.** A PD layer is nearly memoryless: given the latest target, position, and velocity, it produces a torque. An ADRC layer carries observer states $(\hat{q}, \dot{\hat{q}}, \hat{f})$. Those states need initialization, reset behavior after falls or impacts, and consistent treatment during mode switches. That is manageable in an industrial servo. It is more awkward in a learned locomotion stack where thousands of randomized simulations train the policy to exploit a very specific low-level actuator model.

This does not mean ADRC is bad robotics control. It means the best insertion point matters.

ADRC is attractive when the robot is doing **classical servo work**: a manipulator joint tracking a trajectory under unknown payload, a mobile robot rejecting slope and slip, a quadrotor rejecting wind and arm-coupled disturbances, or an industrial axis whose friction and load vary across the day. There the reference is smooth, the actuator model is stable, and the objective is exactly what ADRC is good at: make the plant look simple despite uncertainty.

ADRC is less obviously attractive as a drop-in replacement for the low-level PD layer in modern RL locomotion. The learned policy and the PD actuator form one trained object. Replacing PD with ADRC after training changes the actuator dynamics that the policy was optimized against; a stronger classical servo is not automatically a better policy interface.

The honest conclusion is nuanced: **ADRC belongs in robotics, but not automatically at every joint of every humanoid.** It shines when disturbance rejection is the main job. In current sim-to-real locomotion, the main job of the joint controller is often different: provide a simple, repeatable, compliant actuator interface for a policy trained around that interface.
...
- Fareh, R., Khadraoui, S., Abdallah, M.Y., Baziyad, M., & Bettayeb, M. (2021). "Active disturbance rejection control for robotic systems: A review." *Mechatronics*, 80. — Survey of ADRC applications in robot manipulators, mobile robots, and aerial robots.

**Robotics and learned locomotion context:**
- Rudin, N., Hoeller, D., Reist, P., & Hutter, M. (2022). "Learning to Walk in Minutes Using Massively Parallel Deep Reinforcement Learning." *Conference on Robot Learning.* — The `legged_gym` lineage: massively parallel sim-to-real legged locomotion with a learned policy and low-level actuator model.
- Gu, X., Wang, Y.J., & Chen, J. (2024). "Humanoid-Gym: Reinforcement Learning for Humanoid Robot with Zero-Shot Sim2Real Transfer." *arXiv:2404.05695.* — Humanoid locomotion framework built on the same sim-to-real idea, verified on XBot-S and XBot-L.
